...
int main()
{
    File file("/Users/boone/Desktop/Music/Seve.wav");
    file.WAV.data_size;
    ...
    if (in==NULL||out==NULL) {
        ...
        int  i=0;
        for(vector<float>::iterator it = file.vertices.begin(); it != file.vertices.end(); it+=2 ){
            in[i][0]=*it;
            in[i][1]=0;
            i++;
        }
    ...
    xstart=-1.0;
    int i=0;
    for(vector<float>::iterator it = file.vertices.begin(); it != file.vertices.end(); it+=2 )    //用迭代器的方式输出容器对象的值
    {
        if (*it>0) {
            *it=-*it;
        }
